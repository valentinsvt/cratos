    ...
    <body>
        <div class="ui-widget-content ui-corner-all cont">
            ...
            <div id="list-bodega" class="content scaffold-list" role="main">
                ...
                <table id="tbl-bodega">
                    <thead>
                        <tr>
                            ...
                            <th><g:message code="bodega.centroCosto.label" default="Centro Costo" /></th>
                            
                            <g:sortableColumn property="codigo" title="${message(code: 'bodega.codigo.label', default: 'Codigo')}" />
                    ...
                    <tbody id="tb-bodega">
                        <g:each in="${bodegaInstanceList}" status="i" var="bodegaInstance">
                            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}" id="${bodegaInstance.id}">
                                
                                <td>${fieldValue(bean: bodegaInstance, field: "centroCosto")}</td>
                ...
                <g:if test="${bodegaInstanceList.size() < bodegaInstanceTotal}">
                    <div class="pagination">
                        <g:paginate total="${bodegaInstanceTotal}"  prev="Ant." next="Sig." />
                    </div>
                </g:if>
            </div>
        </div>

        <ul id="menu-bodega" class="contextMenu">
            <li class="show">
                <a href="#show">Ver</a>
            </li>
            <li class="edit">
        ...
        <script type="text/javascript">
            function openDlg(url, id, cont, ajax, title, buttons) {
                ...
                $("#dlgLoad").dialog("open");
                    ...
                    $("#dlg-bodega").dialog("option", "width", 700);
                } else {
                $("#dlg-bodega").html(cont);
                }
                $("#dlg-bodega").dialog("option", "title", title);
                $("#dlg-bodega").dialog("option", "buttons", buttons);
                $("#dlg-bodega").dialog("open");
            }

            function submitForm() {
                if ($("#frm-bodega").valid()) {
                    $("#dlgLoad").dialog("open");
                    var data = $("#frm-bodega").serialize();
                    var url = $("#frm-bodega").attr("action");
            ...
            $(function () {
                ...
                $("th").hover(function () {
                    $(this).addClass("hover");
                    var i = $(this).index();
                    $("#tb-bodega").find("tr").each(function () {
                ...
                $("#tb-bodega").find("tr").hover(function () {
                    $(this).addClass("hover");
                }, function () {
                    $(".hover").removeClass("hover");
                });
                        ...
                        function (action, el, pos) {
                            ...
                            switch (action) {
                                ...
                                case "delete":
                                    ...
                                    cont = "<span style='font-size: 16px;'> Est&aacute; seguro de querer eliminar este Bodega?";
                                    cont += "<br/>Esta acci&oacute;n es definitiva.</span>"
                                    $("#dlg-bodega").dialog("option", "width", 360);
                                    break;
                            }
