    ...
    <body>
        <div class="ui-widget-content ui-corner-all cont">
            ...
            <div id="list-mes" class="content scaffold-list" role="main">
                ...
                <table id="tbl-mes">
                    <thead>
                        <tr>
                            ...
                            <g:sortableColumn property="descripcion" title="${message(code: 'mes.descripcion.label', default: 'Descripción')}" />
                            
                        </tr>
                    </thead>
                    <tbody id="tb-mes">
                        <g:each in="${mesInstanceList}" status="i" var="mesInstance">
                            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}" id="${mesInstance.id}">
                ...
                <g:if test="${mesInstanceList.size() < mesInstanceTotal}">
                    <div class="pagination">
                        <g:paginate total="${mesInstanceTotal}"  prev="Ant." next="Sig." />
                    </div>
                </g:if>
            </div>
        </div>

        <ul id="menu-mes" class="contextMenu">
            <li class="show">
                <a href="#show">Ver</a>
            </li>
            <li class="edit">
        ...
        <script type="text/javascript">
            function openDlg(url, id, cont, ajax, title, buttons) {
                ...
                $("#dlgLoad").dialog("open");
                    ...
                    $("#dlg-mes").dialog("option", "width", 420);
                } else {
                $("#dlg-mes").html(cont);
                }
                $("#dlg-mes").dialog("option", "title", title);
                $("#dlg-mes").dialog("option", "buttons", buttons);
                $("#dlg-mes").dialog("open");
            }
            ...
            function submitForm() {
                if ($("#frm-mes").valid()) {
                    $("#dlgLoad").dialog("open");
                    var data = $("#frm-mes").serialize();
                    var url = $("#frm-mes").attr("action");
                        ...
                        function (action, el, pos) {
                            ...
                            switch (action) {
                                ...
                                case "delete":
                                    ...
                                    cont = "<span style='font-size: 16px;'> Est&aacute; seguro de querer eliminar este Mes?";
                                    cont += "<br/>Esta acci&oacute;n es definitiva.</span>"
                                    $("#dlg-mes").dialog("option", "width", 360);
                                    break;
                            }
