        ...
        <div class="ui-widget-content ui-corner-all cont">
            ...
            <div id="list-nacionalidad" class="content scaffold-list" role="main">
                ...
                <table id="tbl-nacionalidad">
                    ...
                    <tbody id="tb-nacionalidad">
                        <g:each in="${nacionalidadInstanceList}" status="i" var="nacionalidadInstance">
                            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}" id="${nacionalidadInstance.id}">
                ...
                <g:if test="${nacionalidadInstanceList.size() < nacionalidadInstanceTotal}">
                    <div class="pagination">
                        <g:paginate total="${nacionalidadInstanceTotal}"  prev="Ant." next="Sig." />
                    </div>
                </g:if>
            </div>
        </div>

        <ul id="menu-nacionalidad" class="contextMenu">
            <li class="show">
                <a href="#show">Ver</a>
            </li>
            <li class="edit">
        ...
        <script type="text/javascript">
                ...
                $("#dlgLoad").dialog("open");
                    $.ajax({
                        ...
                        success : function (msg) {
                            $("#dlg-nacionalidad").html(msg);
                        },
                        complete : function () {
                            $("#dlgLoad").dialog("close");
                        }
                    });
                    $("#dlg-nacionalidad").dialog("option", "width", 520);
                } else {
                $("#dlg-nacionalidad").html(cont);
                }
                $("#dlg-nacionalidad").dialog("option", "title", title);
                $("#dlg-nacionalidad").dialog("option", "buttons", buttons);
                $("#dlg-nacionalidad").dialog("open");
            }

            function submitForm() {
                if ($("#frm-nacionalidad").valid()) {
                    $("#dlgLoad").dialog("open");
                    var data = $("#frm-nacionalidad").serialize();
                    var url = $("#frm-nacionalidad").attr("action");
            ...
            $(function () {
                ...
                $("th").hover(function () {
                    $(this).addClass("hover");
                    var i = $(this).index();
                    $("#tb-nacionalidad").find("tr").each(function () {
                ...
                $("#tb-nacionalidad").find("tr").hover(function () {
                    $(this).addClass("hover");
                }, function () {
                    $(".hover").removeClass("hover");
                });
                ...
                $(".btnNew").button({
                    icons : {
                        primary : "ui-icon-document"
                            ...
                            var title = "Crear Nacionalidad";
                            var buttons = {
                                "Guardar"  : function () {
                                    submitForm();
                                },
                                "Cancelar" : function () {
                                    $("#dlg-nacionalidad").dialog("close");
                                }
                            };
                ...
                $("#tb-nacionalidad").find("tr").contextMenu({
                            menu : "menu-nacionalidad"
                        },
                        function (action, el, pos) {
                            $("#dlg-nacionalidad").html("");
                            var id = $(el).attr("id");
                            ...
                            switch (action) {
                                ...
                                case "delete":
                                    ...
                                    cont = "<span style='font-size: 16px;'> Est&aacute; seguro de querer eliminar este Nacionalidad?";
                                    cont += "<br/>Esta acci&oacute;n es definitiva.</span>"
                                    $("#dlg-nacionalidad").dialog("option", "width", 360);
                                    break;
                            }
