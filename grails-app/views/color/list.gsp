    ...
    <body>
        <div class="ui-widget-content ui-corner-all cont">
            ...
            <div id="list-color" class="content scaffold-list" role="main">
                ...
                <table id="tbl-color">
                    <thead>
                        <tr>
                            ...
                            <g:sortableColumn property="descripcion" title="${message(code: 'color.descripcion.label', default: 'Descripción')}" />
                            
                        </tr>
                    </thead>
                    <tbody id="tb-color">
                        <g:each in="${colorInstanceList}" status="i" var="colorInstance">
                            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}" id="${colorInstance.id}">
                ...
                <g:if test="${colorInstanceList.size() < colorInstanceTotal}">
                    <div class="pagination">
                        <g:paginate total="${colorInstanceTotal}"  prev="Ant." next="Sig." />
                    </div>
                </g:if>
            </div>
        </div>

        <ul id="menu-color" class="contextMenu">
            <li class="show">
                <a href="#show">Ver</a>
            </li>
            <li class="edit">
        ...
        <script type="text/javascript">
            function openDlg(url, id, cont, ajax, title, buttons) {
                ...
                $("#dlgLoad").dialog("open");
                    ...
                    $("#dlg-color").dialog("option", "width", 420);
                } else {
                $("#dlg-color").html(cont);
                }
                $("#dlg-color").dialog("option", "title", title);
                $("#dlg-color").dialog("option", "buttons", buttons);
                $("#dlg-color").dialog("open");
            }

            function submitForm() {
                if ($("#frm-color").valid()) {
                    $("#dlgLoad").dialog("open");
                    var data = $("#frm-color").serialize();
                    var url = $("#frm-color").attr("action");
            ...
            $(function () {
                ...
                $("th").hover(function () {
                    $(this).addClass("hover");
                    var i = $(this).index();
                    $("#tb-color").find("tr").each(function () {
                ...
                $("#tb-color").find("tr").hover(function () {
                    $(this).addClass("hover");
                }, function () {
                    $(".hover").removeClass("hover");
                });
                ...
                $(".btnNew").button({
                    icons : {
                        primary : "ui-icon-document"
                            ...
                            var title = "Crear Color";
                            var buttons = {
                                "Guardar"  : function () {
                                    submitForm();
                                },
                                "Cancelar" : function () {
                                    $("#dlg-color").dialog("close");
                                }
                            };
                        ...
                        function (action, el, pos) {
                            ...
                            switch (action) {
                                ...
                                case "delete":
                                    ...
                                    cont = "<span style='font-size: 16px;'> Est&aacute; seguro de querer eliminar este Color?";
                                    cont += "<br/>Esta acci&oacute;n es definitiva.</span>"
                                    $("#dlg-color").dialog("option", "width", 360);
                                    break;
                            }
